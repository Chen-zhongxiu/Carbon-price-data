...
online_eval_one_step <- function(X_full, fit0, N_state,
  ...
  # --- Determine batch_size: if not provided, try to infer from fit0$n_batch ---
  if (is.null(batch_size)) {
    n_batch <- fit0$n_batch
    if (is.null(n_batch)) stop("Please pass batch_size, or make sure fit0$n_batch exists.")
  ...
  # Output containers: prediction targets t+1, so t=1..Tn yields S_pred[t+1]
...
T_all = length(data1$price)
T_test  <- 3
T_train <- T_all - T_test

X_full = data1$log_price
