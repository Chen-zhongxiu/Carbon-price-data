...
###########################################################
## 4*. Forward-backward algorithm (RN-derivative + user-specified initial distribution)
##
##    init_logpi: log initial distribution vector of length M
##    - NULL: falls back to the "uniform prior" (-log M)
##    - In batch mode: use the last gamma[,T] from previous batch as the initial distribution
###########################################################
...
fb_augmented_mixture_rn <- function(dx, A, params, N,
  ...
  ## Initial step: optional custom initial distribution
  if (is.null(init_logpi)) {
    # uniform prior
    log_alpha[, 1] <- -log(M) + logB[, 1]
  } else {
    # user provides log π_0(i)
...
###########################################################
## Batch + global-statistics version of EM
##
##  - X: log-price path
##  - n_batch: split diff(X) evenly into n_batch chunks
##  - Returns: final parameters + parameter path per batch (history)
###########################################################
...
em_fit_jump_mixture_rn_batch_global <- function(
  ...
  # Split Tn increments into n_batch segments (as evenly as possible)
  # batch b corresponds to idx_start[b] : idx_end[b]
  idx_start <- floor((0:(n_batch-1)) * Tn / n_batch) + 1
  idx_end   <- floor((1:n_batch)   * Tn / n_batch)
  
  # Initial parameters: convert (μ, σ) to (ϑ, ϱ^2)
  ...
  # --------- global statistics accumulators (length N) ---------
  W      <- numeric(N)  # ∑ w_a
  W0     <- numeric(N)  # ∑ w0
  W1     <- numeric(N)  # ∑ w1
  S0_dx  <- numeric(N)  # ∑ w0 * dx
  S0_dx2 <- numeric(N)  # ∑ w0 * dx^2
  S1_dx  <- numeric(N)  # ∑ w1 * dx
  S1_dx2 <- numeric(N)  # ∑ w1 * dx^2
  
  # Global transition statistics: ∑ ξ_t
  Xi_global <- matrix(0, M, M)
  
  # End-of-previous-batch state distribution (in augmented state space)
  init_logpi <- rep(-log(M), M)  # first batch: uniform
  
  # Parameter path per batch
  ...
  for (b in 1:n_batch) {
    ...
    # --- E-step: use only the data in this batch under current parameters ---
    A  <- build_A_from_p(p)
    ...
    gamma_b  <- fb$gamma        # M x T_b
    xi_sum_b <- fb$xi_sum       # M x M
    Xi_global <- Xi_global + xi_sum_b
    
    # Jump posterior & state marginal prob (this batch)
    stats_b  <- jump_posterior(dx_b, params, N, gamma_b)
    r_b      <- stats_b$r        # N x T_b
    a_prob_b <- stats_b$a_prob   # N x T_b
    ...
    # --- M-step: update parameters using global statistics up to this batch ---
    ...
    for (a in 1:N) {
      ...
      # s_{βa}^2: second moment using w1, minus diffusion variance
      if (W1[a] > 0) {
    ...
    # Transition probabilities: use global Xi_global directly
    p <- mstep_transition(Xi_global, N)
    
    # --- save end-of-batch parameters (useful for plotting trajectories/comparisons) ---
    ...
    # --- prepare initial state distribution for the next batch ---
    gamma_end <- gamma_b[, T_b]  # augmented-state distribution at t = end of batch
    gamma_end <- pmax(gamma_end, 1e-300)
    gamma_end <- gamma_end / sum(gamma_end)
    init_logpi <- log(gamma_end)
  }
  
  # Final parameters (params and p after the last batch)
...
df <- simulate_jumpdiff_01(
  ...
  X0 = 0, seed = 2025, return_price = FALSE
)
...
mu_beta_init <- c(-0.05, -0.20)
s_beta_init  <- c(0.15, 0.25)

mu_init      <- c(0.2145,0.3234)
sigma_init   <- c(0.1455 , 0.2456)
lambda_init  <- c(0.2, 0.6)
mu_beta_init <- c(-0.05, -0.20)
s_beta_init  <- c(0.15, 0.25)
...
fit0  <- em_fit_jump_mixture_rn_batch_global(
  ...
  n_batch = 50,
  esscher_theta = 0.5,   # can change to 0.5, 1 to check robustness
  verbose = FALSE
)

# Suppose the object is called p and it has p$history
...
# Export to CSV (no row names)
#write.csv(df, file = "0120_convergence_path_0.5.csv", row.names = FALSE)


fit_pos <- em_fit_jump_mixture_rn_batch_global(
  X = X, dt = dt, N = N,
  p_init = p_init,
  mu_init = mu_init, sigma_init = sigma_init,
  lambda_init = lambda_init,
  ...
  s_beta_init  = s_beta_init,
  n_batch = 50,
  esscher_theta = 0.3,   # can change to 0.5, 1 to check robustness
  verbose = FALSE
)
...
df = t(df)
# Export to CSV (no row names)
#write.csv(df, file = "history0.3.csv", row.names = FALSE)

fit_neg <- em_fit_jump_mixture_rn_batch_global(
  X = X, dt = dt, N = N,
  p_init = p_init,
  mu_init = mu_init, sigma_init = sigma_init,
  lambda_init = lambda_init,
...
df = t(df)
# Export to CSV (no row names)
write.csv(df, file = "history0.5.csv", row.names = FALSE)
## ==============================
...
run_one_mc <- function(
  ...
  p_init[,2,1] <- c(0.55, 0.45)
  p_init[,1,2] <- c(0.35, 0.65)
  p_init[,2,2] <- c(0.10, 0.90)
  
  # ---- 3. EM estimation ----
  fit <- em_fit_jump_mixture_rn_batch_global(
    X = X, dt = dt, N = N,
    p_init = p_init,
    mu_init = mu_init, sigma_init = sigma_init,
    lambda_init = lambda_init,
    ...
    s_beta_init  = s_beta_init,
    n_batch = 50,
    verbose = FALSE,esscher_theta = esscher_theta 
  )
...
my_algo <- function() {
  T  <- 1000
  dt <- 1
  R  <- 100  # set to 1000 if you need it
