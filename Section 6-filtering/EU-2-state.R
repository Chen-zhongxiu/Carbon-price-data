...
mu_init      <- c(0.00031,0.000445)
sigma_init   <- c(0.0155 , 0.03456)
lambda_init  <- c(0.0132, 0.03556)
mu_beta_init <- c(0.002, 0.004)
s_beta_init  <- c(0.6435, 0.8342)
...
#writexl::write_xlsx(df_a,"D:/博士/论文1/审稿意见/数据/zong.xlsx")
#df_a = readxl::read_xlsx("D:/博士/论文1/审稿意见/数据/zong.xlsx")
#df_a = data.frame(date,df_a[,2:3])
a =  ggplot(data = df_a,aes(x=date,y=df_a[,2]))+
  ...
  scale_color_manual(values = c("State 1" = "black", "State 2" = "red"))+
  labs(x = NULL)+
  ylab(expression(mu))+
  theme_bw()  +
...
ggarrange(plot_0,a,b,c,d,e,
          ...
          labels = c("","F", "G","H","I","J"),label.y = 1.1)

#ggsave("EUA_2_state_par_1.png",width =7,height=15,dpi = 500)
...
data = df[,12:19]
data = as.matrix(data)
colnames(data) = c("p111","p211","p112","p212","p121","p221","p122","p222")
rownames(data) = date
datap <- melt(data)
colnames(datap) = c("x","ci","G")
datap$ci <- factor(datap$ci, levels=c("p111","p211","p112","p212","p121","p221","p122","p222"))
datap$x = as.Date(datap$x)
ggplot(data = datap,aes(x=x,y=G))+
  ...
  scale_x_date(date_breaks="1 year",date_labels="%Y-%m",expand = c(0, 0))+#更改横坐标刻度值
  scale_y_continuous(limits = c(0,1),breaks = seq(0,1,0.2),expand = c(0, 0))
#ggsave("EUA_2_state_Tran_1.png",width =7,height=4,dpi = 500)

#状态估计图
df_g = data.frame(date =as.Date(data1$日期[-1]),fit0$state_prob[1,],fit0$state_prob[2,])
end_dates_data = data.frame(date = end_dates)
df_h = left_join(end_dates_data,df_g,by = "date")

ggplot(data = df_g,aes(x=date,y=df_g[,2]))+
  geom_line(linetype = "solid",size=0.5,aes(color = "State 1"))+
  geom_point(aes(x=date,y=df_g[,2],color = "State 1"),size=1.2)+
  geom_line(data=df_g,aes(x=date, y=df_g[,3],color = "State 2"),linetype = "dashed",size=0.5)+
  geom_point(aes(x=date,y=df_g[,3],color = "State 2"),size=1.2,shape=17)+
  scale_color_manual(values = c("State 1" = "black", "State 2" = "red"))+
  labs(x = NULL)+
...
#ggsave("D:/博士/（出国）论文-computational economics/EUA_state_est_2.png",width =7,height=4,dpi = 500)

df_g = data.frame(date =as.Date(data1$日期[-1]),fit0$state_prob_filter[1,],fit0$state_prob_filter[2,])
end_dates_data = data.frame(date = end_dates)
df_h = left_join(end_dates_data,df_g,by = "date")
...
ggplot(data = df_h,aes(x=date,y=df_h[,2]))+
  ...
  scale_x_date(date_breaks="1 year",date_labels="%Y-%m",expand = c(0, 0))+
  scale_y_continuous(breaks = seq(0,1,0.5))
#ggsave("EUA_state_est_1.png",width =7,height=4,dpi = 500)
