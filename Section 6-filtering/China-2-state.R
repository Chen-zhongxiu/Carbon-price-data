...
mu_init      <- c(0.000321,0.0006234)
sigma_init   <- c(0.008455 , 0.01456)
lambda_init  <- c(0.4, 0.6)
mu_beta_init <- c(-0.0006, -0.00080)
s_beta_init  <- c(0.0452, 0.0634)
...
#ggsave("GC_2_state_par_1.png",width =7,height=15,dpi = 500)
...
ggplot(data = datap,aes(x=x,y=G))+
  ...
  scale_x_date(date_breaks="1 year",date_labels="%Y-%m",expand = c(0, 0))+# change x-axis tick labels
  scale_y_continuous(limits = c(0,1),breaks = seq(0,1,0.2),expand = c(0, 0))
#ggsave("GC_2_state_Tran_1.png",width =7,height=4,dpi = 500)

# State estimation plot
df_g = data.frame(date =as.Date(data1$交易日期[-1]),fit0$state_prob_filter[1,],fit0$state_prob_filter[2,])
...
#ggsave("GC_state_est_2_1.png",width =7,height=4,dpi = 500)
